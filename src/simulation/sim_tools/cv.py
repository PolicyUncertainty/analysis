...
def calc_compensated_variation(df_base, df_cf, params, specs):
    ...
    return cv * 100  # return in percent


def calc_adjusted_scale(df_base, df_cf, params, specs, n_agents):
    # First construct the discounted sum of utilities for the counterfactual scenario
    disc_sum_cf = create_disc_sum(df_cf, specs)
    ...
    # cons_scale (household-scale spending), not consumption / cons_scale --
    # see utility_functions_add.py. Must match here to correctly isolate the
    # consumption-utility component of real_util.
    util_cons = (
        df_base["hh_size"]
        * (
