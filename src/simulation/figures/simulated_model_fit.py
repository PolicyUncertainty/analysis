...
def plot_choice_shares_by_partner(
    path_dict,
    specs,
    params,
    model_name,
    ...
    for partner_state, partner_label in enumerate(specs["partner_labels"]):
        fig, axes = plt.subplots(
            2,
            specs["n_choices"],
            figsize=get_figsize(ncols=specs["n_choices"], nrows=2),
        )
        for sex, sex_label in enumerate(specs["sex_labels"]):
            for edu_var, edu_label in enumerate(specs["education_labels"]):
                data_sim_restr = data_sim[
                    ...
                    & (data_sim["partner_state"] == partner_state)
                ]
                data_decision_restr = data_decision[
                    (data_decision["sex"] == sex)
                    & (data_decision["education"] == edu_var)
                    & (data_decision["partner_state"] == partner_state)
                ]
        ...
        fig.savefig(
            plot_folder + f"sim_choices_{partner_label}.png", transparent=True, dpi=300
        )
        # fig.savefig(plot_folder + f"sim_choices_{partner_label}.pdf", dpi=300)
...
def plot_choice_shares_single(
    ...
    if not os.path.exists(plot_folder):
        os.makedirs(plot_folder)
    fig.savefig(plot_folder + f"sim_choices.png", transparent=True, dpi=300)
    fig.savefig(plot_folder + f"sim_choices.pdf", dpi=300)
