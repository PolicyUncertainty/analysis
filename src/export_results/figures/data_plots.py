...
def plot_data_choices(path_dict, lagged=False):
    ...
    for edu in range(specs["n_education_types"]):
        ...
        if not lagged:
            ...
            choice_shares = edu_df.groupby("age")["lagged_choice"].value_counts(normalize=True)
...
def plot_state_by_age_and_type(path_dict, state_vars):
    ...
    fig, axs = plt.subplots(nrows = n_education_types, ncols = n_state_vars, figsize=(15, 5* n_education_types))

    for edu in range(n_education_types):
        edu_df = struct_est_sample[struct_est_sample["education"] == edu]
        for idx, state_var in enumerate(state_vars):
            state_values = edu_df.groupby("age")[state_var].mean().reindex(plot_ages, fill_value=np.nan).values
