...
def enforce_model_choice_restriction(df, specs):
    ...
    _print_filter(before1, len(df), f"left after dropping people who are working after {max_ret_age}")
    ...
    _print_filter(before2, len(df), "left after dropping people are unemployed after the sra and men who work part-time")
    return df
