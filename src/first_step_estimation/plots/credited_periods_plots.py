...
def plot_credited_periods_results(path_dict, specs, show=False, save=False):
    ...
    df = create_credited_periods_est_sample(path_dict, load_data=specs.get('load_data', False))
    ...
    # Create the plot
    plot_credited_periods_vs_exp(df, model, columns, show=show, save=save, path_dict=path_dict)


def plot_credited_periods_vs_exp(df, model, columns, show=False, save=False, path_dict=None):
    """
    ...
    ax.scatter(df[men_mask]["experience"], df[men_mask]["credited_periods"], 
              label="Actual (Men)", color=jet_colors[0], alpha=0.6)
    ax.scatter(df[~men_mask]["experience"], df[~men_mask]["credited_periods"], 
              label="Actual (Women)", color=jet_colors[3], alpha=0.6)
    ax.scatter(df[men_mask]["experience"], df[men_mask]["predicted_credited_periods"], 
              label="Predicted (Men)", color=jet_colors[0], marker='^', s=30)
    ax.scatter(df[~men_mask]["experience"], df[~men_mask]["predicted_credited_periods"], 
              label="Predicted (Women)", color=jet_colors[3], marker='^', s=30)
