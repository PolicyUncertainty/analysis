...
os.makedirs(path_dict["intermediate_data"] + "beliefs/", exist_ok=True)
belief_data_path = path_dict["intermediate_data"] + "beliefs/"
...
df.to_csv(belief_data_path + "soep_is_clean.csv")
...
df_truncated_normal = estimate_truncated_normal(df, paths=path_dict, options=specs, load_data=False)
df.to_csv(belief_data_path + "soep_is_truncated_normals.csv")
...
uninformed_params_df = calibrate_uninformed_hazard_rate_with_se(df, specs, calculate_se=False)

# save results
params_df = pd.concat([sra_params_df, uninformed_params_df], ignore_index=True)
params_df.to_csv(belief_data_path + "beliefs_parameters.csv", index=False)