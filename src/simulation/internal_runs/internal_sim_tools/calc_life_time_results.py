...
def add_new_life_cycle_results(df_base, df_cf, scenatio_indicator, res_df_life_cycle):
    ...
    for append, df_scenario in zip(["base", "cf"], [df_base, df_cf]):
        ...
        res_df_life_cycle[f"working_hours_{scenatio_indicator}_{append}"] = (
            df_scenario.groupby("age")["working_hours"].aggregate("mean")
        )
        ...
        res_df_life_cycle[f"employment_rate_{scenatio_indicator}_{append}"] = (
            df_scenario.groupby("age")["choice"].apply(
                lambda x: x.isin([2, 3]).sum() / len(x)
            )
        )
        res_df_life_cycle[f"retirement_rate_{scenatio_indicator}_{append}"] = (
            df_scenario.groupby("age")["choice"].apply(
                lambda x: x.isin([0]).sum() / len(x)
            )
        )
