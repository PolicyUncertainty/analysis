...
def plot_wage_regression_by_experience(path_dict, specs, show=False, save=False):
    ...
    for sex_val, sex_label in enumerate(sex_labels):
        ...
        for edu_val, edu_label in enumerate(edu_labels):
            ...
            wage_data_type["experience_int"] = wage_data_type["experience"].round().astype(int)
            ...
            predicted_by_exp = wage_data_type.groupby("experience_int")["predicted_ln_wage"].mean()
            ax.plot(
        ...
        if save:
            fig.savefig(
                path_dict["first_step_plots"] + f"wages_by_exp_{file_appends[sex_val]}.png",
                bbox_inches="tight"
            )
            fig.savefig(
                path_dict["first_step_plots"] + f"wages_by_exp_{file_appends[sex_val]}.pdf",
                bbox_inches="tight"
            )
