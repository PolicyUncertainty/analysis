...
def load_gender_results(path_dict, model_name):
    ...
    for scenario in ["unc", "no_unc"]:  # , "debias"]:
        ...
        for gender in ["overall", "men", "women"]:
            filepath = path_dict["sim_results"] + f"sra_increase_aggregate_{scenario}_{gender}_{model_name}.csv"
    ...
    for column in df_prepared.columns.values:
        if column == "sra_at_63":
            ...
            if "base" in column:
                ...
                df_prepared.loc[0, column] = df_prepared.loc[1, "base" + column_name_without_cf]
        else:
...
def plot_retirement_age_changes(ax, df_unc, df_no_unc, title, show_legend=False):
    ...
    ax.plot(df_unc["sra_at_63"], change_sra_unc, color=JET_COLOR_MAP[0], 
            ls="--", label="45 degree")
...
def sra_increase_aggregate_plot_by_gender(path_dict, model_name):
    ...
    for row, gender in enumerate(gender_order):
        ...
        for col, (var, ylabel) in enumerate(variables.items()):
            ax = axs[row, col]
            plot_behavioral_changes(ax, df_unc, df_no_unc, var, ylabel, 
        ...
        plot_retirement_age_changes(ax, df_unc, df_no_unc, 
...
def sra_increase_aggregate_plot(path_dict, model_name):
    ...
    # Retirement age plot
    plot_retirement_age_changes(axs[2], df_unc, df_no_unc, "Retirement Age", show_legend=True)
