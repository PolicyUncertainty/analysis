...
def calc_inc_tax_for_single_income(gross_income, options):
    ...
    gross_income_scaled = gross_income * options["wealth_unit"]
    thresholds = [
        8004,
        13469,
        52881,
    ...
    poss_tax_bracket_1 = rates[0] * (gross_income_scaled - thresholds[0])
    ...
    # bracket 1
    poss_tax_bracket_2 = (
        rates[1] * (gross_income_scaled - thresholds[1])
    ) + tax_above_1
    ...
    # brackets 1+2
    poss_tax_bracket_3 = (
        rates[2] * (gross_income_scaled - thresholds[2]) + tax_above_2 + tax_above_1
    )
    ...
    # brackets 1+2+3
    poss_tax_bracket_4 = (
        rates[3] * (gross_income_scaled - thresholds[3])
        + tax_above_1
        + tax_above_2
        + tax_above_3
    )
    ...
    in_bracket_0 = gross_income_scaled < thresholds[0]
    in_bracket_1 = (gross_income_scaled >= thresholds[0]) & (
        gross_income_scaled < thresholds[1]
    )
    in_bracket_2 = (gross_income_scaled >= thresholds[1]) & (
        gross_income_scaled < thresholds[2]
    )
    in_bracket_3 = (gross_income_scaled >= thresholds[2]) & (
        gross_income_scaled < thresholds[3]
    )
    in_bracket_4 = gross_income_scaled >= thresholds[3]
    ...
    return income_tax / options["wealth_unit"]


def calc_after_ssc_income_worker(gross_wage, options):
    """Calculate the net income after social security contributions."""
    income_scaled = gross_wage * options["wealth_unit"]
    ssc = calc_pension_unempl_contr(income_scaled) + calc_health_ltc_contr(
        income_scaled
    )
    return gross_wage - ssc / options["wealth_unit"]
