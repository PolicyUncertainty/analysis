...
# Delete all parameters with "_women" in name
list_of_keys = [key for key in last_estimate.keys() if "_women" in key]
for param in list_of_keys:
    del last_estimate[param]

# Now loop over start params and set them to last estimate
for param in start_params_all.keys():
    if "_women" in param:
        last_estimate[param] = start_params_all[param]
