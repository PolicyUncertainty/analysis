...
def est_SRA_params(paths, df=None, print_summary=False):
    ...
    if df is None:
        df = pd.read_csv(paths["beliefs_data"] + "soep_is_truncated_normals.csv")
    df = filter_df(df)
    # estimate expected SRA increase and variance
    alpha_hat, alpha_hat_std_err = est_expected_SRA(
        paths, df, print_summary=print_summary
    )
...
def est_expected_SRA(
    paths, df=None, print_summary=False, covariates=None, include_constant=False
):
    """
    ...
    if print_summary:
        ...
        coef_str = " + ".join(
            [f"{coef:.4f} * {var}" for coef, var in zip(fitted_model.params, var_names)]
        )
...
def est_alpha_heterogeneity(paths, df=None):
    ...
    for covariate in covariates:
        ...
        try:
            ...
            print(
                f"  Univariate regression: {len(df_reg_univ)} observations after cleaning"
            )
            ...
            if len(df_reg_univ) > 0:
                ...
                results_list.append(
                    {
                        ...
                        "t_stat": (
                            covariate_coef_univ / covariate_se_univ
                            if covariate_se_univ != 0
                            else np.nan
                        ),
                ...
                print(
                    f"  No valid observations for univariate regression with {covariate}"
                )
        ...
        try:
            ...
            if age_controls:
                required_cols_ctrl = (
                    ["time_to_ret", covariate]
                    + age_controls
                    + ["exp_SRA_increase", "fweights"]
                )
                ...
                print(
                    f"  Age-controlled regression: {len(df_reg_ctrl)} observations after cleaning"
                )
                ...
                if len(df_reg_ctrl) > 0:
                    params_ctrl, se_ctrl = est_expected_SRA(
                        paths,
                        df_reg_ctrl,
                        covariates=[covariate] + age_controls,
                        include_constant=False,
                    )

                    # Extract covariate coefficient (second coefficient after time_to_ret)
                    covariate_coef_ctrl = (
                        params_ctrl[1] if len(params_ctrl) > 1 else np.nan
                    )
                    ...
                    results_list.append(
                        {
                            ...
                            "t_stat": (
                                covariate_coef_ctrl / covariate_se_ctrl
                                if covariate_se_ctrl != 0
                                else np.nan
                            ),
                    ...
                    print(
                        f"  No valid observations for age-controlled regression with {covariate}"
                    )
            else:
                print(
                    f"  Age variable not available for {covariate} age-controlled regression"
                )
