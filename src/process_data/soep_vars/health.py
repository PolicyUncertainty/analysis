...
def clean_health_create_lagged_state(data):
    ...
    data = data[
        data["health_state"].notna()
    ]  # need to do this again here because spanning the dataframe creates new missing values
    ...
    print(
        str(len(data))
        + " observations left after dropping people with missing lagged health data."
    )
