...
def create_health_state(data):
    ...
    data["health_state_prev"] = data.groupby(["pid"])["health_state"].shift(1)
    data["health_state_next"] = data.groupby(["pid"])["health_state"].shift(-1)
    data.loc[
        (data["health_state"] == 0) & 
        (data["health_state_prev"] == 1) & 
        (data["health_state_next"] == 1),
        "health_state"
    ] = 1
    
    # drop unnecessary columns
    data.drop(["health_state_prev", "health_state_next"], axis=1, inplace=True)
